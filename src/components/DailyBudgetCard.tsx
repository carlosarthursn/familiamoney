...
export function DailyBudgetCard() {
  ...
  const dailyAmount = useMemo(() => {
    ...
    // Se não houver orçamento definido, retorna 0
    if (budget <= 0) return 0;
    
    // Disponível = (Orçamento Mensal - O que já gastei pessoalmente) / Dias que faltam
  ...
  const handleUpdateBudget = async () => {
    ...
    if (error) {
      ...
      toast.success('Orçamento mensal atualizado!');
      setIsEditing(false);
    }
  };
